...
extension TrainerProfileViewController: UnavailableDatesDelegate {
    ...
    private func saveUnavailableDates(_ dates: [Date]) {
        ...
        db.collection("availability").document(trainerId).setData(["unavailableDates": unavailableDatesData]) { error in
            if let error = error {
                print("Error saving unavailable dates: \(error.localizedDescription)")
            } else {
                print("Unavailable dates saved successfully!")
            }
        }
    }
}
